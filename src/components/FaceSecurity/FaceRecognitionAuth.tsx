...
export default function FaceRecognitionAuth({ studentDescriptor, onAuthenticated }: FaceRecognitionAuthProps) {
  ...
  useEffect(() => {
    ...
    // Convert studentDescriptor to Float32Array
    const storedDescriptor = new Float32Array(studentDescriptor);
    const THRESHOLD = 0.6;
    ...
    const detectAndMatchFace = async () => {
      ...
      try {
        ...
        console.log(`Face match distance: ${distance.toFixed(3)} (threshold: ${THRESHOLD})`);
